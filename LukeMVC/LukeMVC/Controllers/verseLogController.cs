...
{
    public class verseLogController : Controller
    {
        ...
        public ActionResult SaveVerseLog(string verseId, string verseCatId)
        {
            verse_category_log vcl = new verse_category_log();
            vcl.verse_category_id = Int32.Parse(verseCatId);
            vcl.verse_id = Int32.Parse(verseId);

            var verseCount = db.verse_category_log
                             .Where(v => v.verse_id == vcl.verse_id)
                             .Count();

            if (verseCount == 0 )
            {
                 db.verse_category_log.Add(vcl);
            }

            else
            {
                verse_category_log log = db.verse_category_log.Find(vcl.verse_id, vcl.verse_category_id);

                db.verse_category_log.Remove(log);
            }                      

            db.SaveChanges();

            return new JsonResult() { Data = new { returnMsg = "success" } };
        }
        ...
        public ActionResult Create([Bind(Include = "verse_id,verse_category_id")] verse_category_log verse_category_log)
        {
            if (ModelState.IsValid)
            {
                ...
                db.SaveChanges();
                return new JsonResult() { Data = new { name = "Hello There" } };//RedirectToAction("Index");
            }
